...
$ajaxRefresh = false;
if(isset($_GET['ouid'])) {
  //ShoprocketSession::drop('ShoprocketPendingOUID')
...
elseif(ShoprocketSession::get('ShoprocketPendingOUID')) {
  $order = new ShoprocketOrder();
  $order->loadByOuid(ShoprocketSession::get('ShoprocketPendingOUID'));
  if(empty($order->id) || $order->status == 'checkout_pending') {
    $ajaxRefresh = true;
  }
  else {
    $url = add_query_arg('ouid', ShoprocketSession::get('ShoprocketPendingOUID'), ShoprocketCommon::getCurrentPageUrl());
    ShoprocketSession::drop('ShoprocketPendingOUID');
    wp_redirect($url);
    exit;
  }
...
if(ShoprocketSession::get('ap_id')) {
  $referrer = ShoprocketSession::get('ap_id');
}
...
if(is_object($order) && $order->viewed == 0){
  ...
  if (!empty($referrer)) {
    ShoprocketCommon::awardCommission($order->id, $referrer);
  }
  // End processing affiliate information

  // Begin iDevAffiliate Tracking
  if(false && $url = ShoprocketSetting::getValue('idevaff_url')) {
    require_once(SHOPROCKET_PATH . "/pro/idevaffiliate-award.php");
  }
  ...
  if($product->loadByDuid($duid)) {
    ...
    if($okToDownload) {
      ...
      if(!empty($product->s3Bucket) && !empty($product->s3File)) {
        require_once(SHOPROCKET_PATH . '/models/ShoprocketAmazonS3.php');
        $link = ShoprocketAmazonS3::prepareS3Url($product->s3Bucket, $product->s3File, '1 minute');
        wp_redirect($link);
        exit;
      }
      else {
      ...
      echo '<p>' . __("You have exceeded the maximum number of downloads for this product","shoprocket") . '.</p>';
      $order = new ShoprocketOrder();
...
if(!$ajaxRefresh) :
  if(ShoprocketSetting::getValue('enable_google_analytics') == 1 && !ShoprocketSetting::getValue('use_other_analytics_plugin')): ?>
    <script type="text/javascript">
      /* <![CDATA[ */
      var _gaq = _gaq || [];
      _gaq.push(['_setAccount', '<?php echo ShoprocketSetting::getValue("google_analytics_wpid") ?>']);
  ...
  <?php  if($order !== false): ?>
  <h2><?php _e( 'Order Number' , 'shoprocket' ); ?>: <?php echo $order->trans_id ?></h2>

  <?php 
  if(false && $order->hasAccount() == 1) {
    $logInLink = ShoprocketAccessManager::getLogInLink();
    $memberHomePageLink = ShoprocketAccessManager::getMemberHomePageLink();
    if($logInLink !== false) {
      echo '<h2>Your Account Is Ready</h2>';
      if(ShoprocketCommon::isLoggedIn() && $memberHomePageLink !== false) {
        echo "<p><a href=\"$memberHomePageLink\">" . __("Members Home","shoprocket") . "</a>.</p>";
      }
      else {
        echo "<p><a href=\"$logInLink\">" . __("Log into your account","shoprocket") . "</a>.</p>";
      }
    }
  }
  ?>
    ...
    <?php if(!ShoprocketCommon::isLoggedIn()): ?>
      ...
      <?php
        if(isset($data['errors'])) {
          ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Account creation errors: " . print_r($data, true));
          echo ShoprocketCommon::showErrors($data['errors'], 'Your account could not be created.');
          echo ShoprocketCommon::getJqErrorScript($data['jqErrors']);
        }
      ?>
      ...
      <form action="" method='post' id="account_form" class="phorm2">
        ...
        <ul class="shortLabels">
          ...
          <li>
            <label for="ShoprocketCheckoutButton" class="ShoprocketHidden"><?php _e( 'Save' , 'shoprocket' ); ?></label>
            <input id="ShoprocketCheckoutButton" class="ShoprocketButtonPrimary ShoprocketCompleteOrderButton" type="submit"  
              value="<?php _e( 'Create Account' , 'shoprocket' ); ?>" name="Create Account"/>
          </li>
        </ul>
      </form>
    <?php endif; ?>
  ...
  <table border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
      <td valign="top">
        <p>
          <strong><?php _e( 'Billing Information' , 'shoprocket' ); ?></strong><br/>
      ...
      <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
      <td valign="top">
        <p><strong><?php _e( 'Contact Information' , 'shoprocket' ); ?></strong><br/>
        ...
        <?php _e( 'Date' , 'shoprocket' ); ?>: <?php echo date(get_option('date_format'), strtotime($order->ordered_on)) ?> <?php echo date(get_option('time_format'), strtotime($order->ordered_on)) ?>
      ...
      <td>
        <?php if($order->shipping_method != 'None'): ?>
          <?php if($order->hasShippingInfo()): ?>
            ...
            <p><strong><?php _e( 'Shipping Information' , 'shoprocket' ); ?></strong><br/>
            ...
            <?php 
              ...
              foreach($tracking as $key => $value) {
                ...
                <div id="tracking_<?php echo $i; ?>_<?php echo $carrierName; ?>" class="tracking_number">
                  <span class="carrier_<?php echo $carrierName; ?>"><?php echo $carrierName ?></span><span class="tracking_text"> <?php _e("Tracking Number","shoprocket") ?></span><span class="tracking_divider">:</span>
      ...
      <tr>
        <td colspan="3">
          <?php if(ShoprocketSetting::getValue('checkout_custom_field_label')): ?>
            <p><strong><?php echo ShoprocketSetting::getValue('checkout_custom_field_label'); ?></strong><br/>
          <?php else: ?>
            <p><strong><?php _e('Enter any special instructions you have for this order:', 'shoprocket'); ?></strong><br/>
          <?php endif; ?>
  ...
  <table id='viewCartTable' cellspacing="0" cellpadding="0">
    <tr>
      <th style='text-align: left;'><?php _e( 'Product' , 'shoprocket' ); ?></th>
      <th style='text-align: center;'><?php _e( 'Quantity' , 'shoprocket' ); ?></th>
      <th style='text-align: left;'><?php _e( 'Item Price' , 'shoprocket' ); ?></th>
      <th style='text-align: left;'><?php _e( 'Item Total' , 'shoprocket' ); ?></th>
    </tr>
    <?php if(ShoprocketSetting::getValue('enable_google_analytics') == 1 && $order->viewed == 0): ?>
      <script type="text/javascript">
        /* <![CDATA[ */
  	    _gaq.push(['_addTrans',
      ...
      <tr>
        <td>
          <?php if(ShoprocketSetting::getValue('display_item_number_cart')): ?>
            <span class="shoprocket-receipt-item-number"><?php echo $item->item_number; ?></span>
          <?php endif; ?>
          ...
          <?php
            ...
            if($product->isDigital()) {
              ...
              echo '<br/><a class="download-link" href="' . $receiptPageLink . '">' . __('Download', 'shoprocket') . '</a>';
            }
          ?>

        </td>
        <td style='text-align: center;'><?php echo $item->quantity ?></td>
        <td><?php echo ShoprocketCommon::currency($item->product_price) ?></td>
        <td><?php echo ShoprocketCommon::currency($item->product_price * $item->quantity) ?></td>
      </tr>
      <?php
          ...
          foreach($entries as $entryId) {
            if(class_exists('RGFormsModel')) {
              if(RGFormsModel::get_lead($entryId)) {
                echo "<tr><td colspan='4'><div class='ShoprocketGravityFormDisplay'>" . ShoprocketGravityReader::displayGravityForm($entryId) . "</div></td></tr>";
              }
            }
            else {
    ...
    <tr class="noBorder">
      ...
      <td colspan="1" style='text-align: right; font-weight: bold;'><?php _e( 'Subtotal' , 'shoprocket' ); ?>:</td>
      <td colspan="1" style="text-align: left; font-weight: bold;"><?php echo ShoprocketCommon::currency($order->subtotal); ?></td>
    </tr>
    ...
    <tr class="noBorder">
      <td colspan='1'>&nbsp;</td>
      <td colspan="1" style='text-align: center;'>&nbsp;</td>
      <td colspan="1" style='text-align: right; font-weight: bold;'><?php _e( 'Shipping' , 'shoprocket' ); ?>:</td>
      <td colspan="1" style="text-align: left; font-weight: bold;"><?php echo ShoprocketCommon::currency($order->shipping); ?></td>
    </tr>
    <?php endif; ?>
    ...
    <?php if($order->discount_amount > 0): ?>
      <tr class="noBorder">
        <td colspan='2'>&nbsp;</td>
        <td colspan="1" style='text-align: right; font-weight: bold;'><?php _e( 'Discount' , 'shoprocket' ); ?>:</td>
        <td colspan="1" style="text-align: left; font-weight: bold;">-<?php echo ShoprocketCommon::currency($order->discount_amount); ?></td>
      </tr>
    <?php endif; ?>
    ...
    <?php if($order->tax > 0): ?>
      <tr class="noBorder">
        <td colspan='2'>&nbsp;</td>
        <td colspan="1" style='text-align: right; font-weight: bold;'><?php _e( 'Tax' , 'shoprocket' ); ?>:</td>
        <td colspan="1" style="text-align: left; font-weight: bold;"><?php echo ShoprocketCommon::currency($order->tax); ?></td>
      </tr>
    <?php endif; ?>
  
    <tr class="noBorder">
      <td colspan='2' style='text-align: center;'>&nbsp;</td>
      <td colspan="1" style='text-align: right; font-weight: bold;'><?php _e( 'Total' , 'shoprocket' ); ?>:</td>
      <td colspan="1" style="text-align: left; font-weight: bold;"><?php echo ShoprocketCommon::currency($order->total); ?></td>
    </tr>
  </table>

  <p><a href='#' id="print_version"><?php _e( 'Printer Friendly Receipt' , 'shoprocket' ); ?></a></p>

  <?php if(ShoprocketSetting::getValue('enable_performance_based_integration')): ?>
  ...
  <?php include(SHOPROCKET_PATH . '/views/newsletter-signup.php'); ?>
  <!-- End Newsletter Signup Form -->

  <?php
    // Erase the shopping cart from the session at the end of viewing the receipt
    ShoprocketSession::drop('ShoprocketCart');
    ShoprocketSession::drop('PayPalProToken');
    ShoprocketSession::drop('ShoprocketTax');
  ...
  ?>
  <?php else: ?>
    <p><?php _e( 'Receipt not available' , 'shoprocket' ); ?></p>
  <?php endif; ?>
  ...
  <?php
    ...
    ?>
    <?php 
    if(ShoprocketSetting::getValue('enable_google_analytics') == 1): ?>
      <?php
      ...
      ?>
      <?php if(!ShoprocketSetting::getValue('use_other_analytics_plugin')): ?>
        <script type="text/javascript">
          /* <![CDATA[ */
            (function() {
    ...
    <?php endif; ?>
<?php else: ?>
  <div class="shoprocket-align-center">
    <?php
    $ajaxLoaderImg = '';
    ...
    ?>
    <?php if($cartImgPath && ShoprocketCommon::urlIsLIve($ajaxLoaderImg)): ?>
      <img src="<?php echo $ajaxLoaderImg; ?>" />
    <?php else: ?>
      <img src="<?php echo SHOPROCKET_URL; ?>/images/ajax-loader.gif" />
    <?php endif; ?>
  ...
  <input type="hidden" name="ouid" id="ouid" value="<?php echo ShoprocketSession::get('ShoprocketPendingOUID'); ?>" />
  <input type="hidden" name="current-page" id="current-page" value="<?php echo ShoprocketCommon::getCurrentPageUrl(); ?>" />
  <script type="text/javascript">
    (function($){
      $(document).ready(function(){
