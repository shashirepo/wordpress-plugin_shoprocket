<?php
class ShoprocketShippingMethod extends ShoprocketModelAbstract {
  
  public function __construct($id=null) {
    $this->_tableName = ShoprocketCommon::getTableName('shipping_methods');
  ...
  public function pruneCarrierMethods($carrier, array $codes) {
    ...
    ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Codes array map: " . print_r($codes, true));
    $codes = implode("','", $codes);
    ...
    ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Pruning shipping methods: $sql");
    $this->_db->query($sql);
  }
  
  public function isLiveMethod($id=null) {
    $id = empty($id) ? $this->id : $id;
    $m = new ShoprocketShippingMethod($id);
    return !empty($m->code);
  }
