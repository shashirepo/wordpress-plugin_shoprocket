<?php
class ShoprocketButtonManager {

  /**
   * Return the HTML for rendering the add to cart buton for the given product id
   */
  public static function getCartButton(ShoprocketProduct $product, $attrs) {
    $view = "<p>" . __("Could not load product information","shoprocket") . "</p>";
    if($product->id > 0) {
      ...
      $buttonText = (isset($attrs['text'])) ? $attrs['text'] : __('Add to Cart', 'shoprocket');
      ...
      if($showPrice == 'yes' || $showPrice == 'only') {
        ...
        if($product->isSubscription()) {
          if($product->isPayPalSubscription()) {
            $subscription = 1;
            ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Rendering button for PayPal subscription");
            $sub = new ShoprocketPayPalSubscription($product->id);
      ...
      $view = ShoprocketCommon::getView('views/cart-button.php', $data, true, true);
    }
    return $view;
  }
  ...
  public static function getAddToCartImagePath($attrs) {
    ...
    if(isset($attrs['img'])) {
      ...
      if($cartImgPath) {
        $cartImgPath = ShoprocketCommon::endSlashPath($cartImgPath);
