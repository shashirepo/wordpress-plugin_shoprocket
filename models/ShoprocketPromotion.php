<?php
class ShoprocketPromotion extends ShoprocketModelAbstract {
  
  public function __construct($id=null) {
    $this->_tableName = ShoprocketCommon::getTableName('promotions');
  ...
  public function getAmountDescription() {
    ...
    if($this->id > 0) {
      if($this->type == 'dollar') {
        $amount = ShoprocketCommon::currency($this->amount) . ' off';
  ...
  public function getMinOrderDescription() {
    $min = $this->minOrder;
    if($min > 0) {
      $min = ShoprocketCommon::currency($min);
    }
    else {
      $min = __('Apply to All Orders', 'shoprocket');
    }
    return $min;
  }
  ...
  public function getMaxOrderDescription() { // CHANGE AND COMBINE WITH ABOVE
    $max = $this->maxOrder;
    if($max > 0) {
      $max = ShoprocketCommon::currency($max);
    }
    else {
      $max = __('Apply to All Orders', 'shoprocket');
    }
    return $max;
  }
  ...
  public function getCode($trim=15,$allValid=false){
     ...
     if(ShoprocketSession::get('ShoprocketPromotionCode') && !$allValid){
       $output = ShoprocketSession::get('ShoprocketPromotionCode');
     }
     else{
       $output = $codes;
     }
  ...
  public function effectiveDates() {
    ...
    if(empty($from) || $from == "0000-00-00 00:00:00") {
      $from = __('No Start Date', 'shoprocket');
    } else {
    ...
    if(empty($to) || $to == "0000-00-00 00:00:00") {
      $to = '<br />' . __('No End Date', 'shoprocket');
    } else {
  ...
  public function getItemPriceAfterPromotion($product_id){
    $product = new ShoprocketProduct($product_id);
  ...
  public function save($saveOverride=false) {
    ...
    if(count($errors) == 0 || $saveOverride) {
      ...
      throw new ShoprocketException('Promotion save failed: ' . $errors, 66301);
    }
    return $promotionId;
  }
  ...
  public function validateCustomerPromotion($code){
    ...
    if(!$customerPromo){
      ...
      $this->_errors[] = __('That promotion is not valid.','shoprocket');
      return false;
    }
    
    if(!$this->shippingExists()) {
      $this->_errors[] = __('No shipping cost is defined.', 'shoprocket');
    }
    
    if(!$this->isEnabled()){
      $this->_errors[] = __('That promotion is not enabled.','shoprocket');
    }  
    
    if(!$this->minAmountMet()){
      $this->_errors[] = __('The minimum required order total for that promotion has not been met.','shoprocket');
    }
    
    if(!$this->maxAmountMet()){
      $this->_errors[] = __('The maximum required order total for that promotion has not been met.','shoprocket');
    }
    
    if(!$this->minQuantityMet()){
      $this->_errors[] = __('The minimum quantity required for that promotion has not been met.','shoprocket');
    }
    
    if(!$this->maxQuantityMet()){
      $this->_errors[] = __('The maximum quantity allowed for that promotion has been exceeded.','shoprocket');
    }
    
    if(!$this->isEffective()){
      $this->_errors[] = __('This promotion is not currently active.','shoprocket');
    }  
    
    if(!$this->withinMaximumRedemptions()){
      $this->_errors[] = __('This promotion has been redeemed the maximum number of times allowed.','shoprocket');
    }
    
    if(!$this->eligibleProductsInCart()){
      $this->_errors[] = __('Your cart does not contain any eligible products for this promotion.','shoprocket');
    }
    ...
    if(empty($this->_errors)){
      ...
      //$this->_errors[] = __('The promotion: ' . $code . ' could not be applied.','shoprocket');
      $output = false;
    }  
  ...
  public function validate() {
    ...
    if(empty($this->name)) {
      $errors['name'] = __('Promotion Name is required', 'shoprocket');
    }
    
    if(empty($this->code)) {
      $errors['code'] = __('Promotion Code is required', 'shoprocket');
    }
    ...
    if(($this->effective_from != null && $this->effective_from != "0000-00-00 00:00:00") && 
      ...
      $errors['effective'] = __(' The Effective Dates are invalid, the ending date must be after the starting date.' . $this->effective_to, 'shoprocket');
    }
    ...
    if(empty($errors)) {
      ...
      foreach($codes as $code){
        ...
        //echo "$code: $exists"; 
        if($exists && $code!=""){
            $errors['code'] = __('The Promotion Code(s) must be unique. The code "'.strtoupper($code).'" is already in use.', 'shoprocket');         
        }
      }
      ...
      if($count > 0) {
        $errors['code'] = __('The Promotion Code(s) must be unique', 'shoprocket');
      }
      */
    }
  ...
  public function eligibleProductsInCart() {
    ...
    $shoprocketCart = ShoprocketSession::get('ShoprocketCart');
    $cartProducts = $shoprocketCart->getProductsAndIds();
  ...
  public function shippingExists() {
    $shippingExists = true;
    if($this->apply_to == 'shipping' && ShoprocketSession::get('ShoprocketCart')->getShippingCost() == 0) {
  ...
  public function minAmountMet() {
    $minAmountMet = false;
    if($this->min_order <= ShoprocketSession::get('ShoprocketCart')->getNonSubscriptionAmount()) { 
  ...
  public function maxAmountMet() {
    $maxAmountMet = false;
    if($this->max_order >= ShoprocketSession::get('ShoprocketCart')->getNonSubscriptionAmount() || $this->max_order == null || $this->max_order == '0' . ShoprocketSetting::getValue('currency_dec_point') . '00') { 
      //_promotion and getNonSubscriptionAmount are cart functions
      $maxAmountMet = true;
    }
    return $maxAmountMet;
  ...
  public function minQuantityMet(){
    ...
    $cartProducts = ShoprocketSession::get('ShoprocketCart')->getItems();
  ...
  public function discountTotal($total) {
    ...
    // ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Calculated discount total: $total");
    return $total;
  }
  ...
  public function getDiscountAmount($cartObject=null, $taxed_products=false){
    ...
    //ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Running getDiscountAmount()");
    $discount = 0; 
    if(!$cartObject){
     $cartObject = ShoprocketSession::get('ShoprocketCart');
    }
    ...
    if($this->apply_to == "products" && !empty($cartObject)) {
     // coupon applies to products
      ...
      ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] The number of items in the cart: " . count($cartItems));
      $usedThisOrder = 0;
          ...
          if(!$taxed_products || ($taxed_products && $p->taxable == 1)) {
            ...
            for($i=1; $i <= $quantity; $i++) {
              if(empty($this->max_uses_per_order)){
                ...
                ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Max uses per order is empty. Discount: $discount");
              }
              elseif($counter < ($this->max_uses_per_order)) {
                $discount+= $stayPositivePrice;
                ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Max uses per order is NOT empty. Discount: $discount :: Stay Positive: $stayPositivePrice");
              }
              $counter++;
            }
          }
    ...
    if($this->apply_to == "subtotal") {
      ...
      if(empty($this->products)) {
        ...
        ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] item subtotal: $itemSubtotal");
  ...
  public function resetPromotionStatus() {
    ...
    $this->getPromotion();
    if(is_a($this->_promotion, 'ShoprocketPromotion')) {
  ...
  public function stayPositive($deductFrom, $discount=null) {
    ...
    ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Stay Positive Amount: $amount :: Deduct From: $deductFrom :: Discount: $discount");
    return $amount;
  }
