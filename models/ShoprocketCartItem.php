...
class ShoprocketCartItem {
  ...
  public function __construct($productId=0, $qty=1, $optionInfo='', $priceDifference=0, $productUrl='') {
    ...
    // ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] New Cart Item Option Info: $optionInfo");
  }
  
  public function getPriceDifference($optionInfo){
    $output = 0;
    if(strlen($optionInfo) > 1){
      $product = new ShoprocketProduct($this->_productId);
  ...
  public function getOptionInfo() {
    ...
    if($this->isSubscription()) {
      ...
      //ShoprocketCommon::log("This is a subscription product: $options " . $this->getItemNumber());
    }
    return $options;
  }
        ...
        if($product->gravity_form_id > 0) {
          ...
          if(count($this->_formEntryIds) == 0) {
            ...
            if($product->gravity_form_qty_id > 0) {
              ...
              $gr = new ShoprocketGravityReader();
  ...
  public function getCustomField($itemIndex, $fullMode=true) {
    ...
    if($this->_productId > 0) {
      ...
      $errorClass = '';
      if(is_array(ShoprocketSession::get('ShoprocketCustomFieldWarning')) && in_array($p->name, ShoprocketSession::get('ShoprocketCustomFieldWarning'))) {
        $errorClass = ' ShoprocketErrorField';
      }
      if($p->custom == 'single') {
        $desc = $p->custom_desc;
        ...
        if($fullMode) {
          ...
          <br/><p class=\"ShoprocketCustomFieldDesc\">$desc:<br/><strong>$value</strong> $change</p>
          <div id='customForm_$itemIndex' style='display: $showCustomForm;'>
          <input type=\"text\" name=\"customFieldInfo[$itemIndex]\" value=\"$value\" class=\"ShoprocketCustomTextField" . $errorClass . "\" id=\"custom_field_info_$itemIndex\" />
          ...
          $out = "<br/><p class=\"ShoprocketCustomFieldDesc\">$desc:<br/><strong>$value</strong></p>";
      ...
      elseif($p->custom == 'multi') {
        ...
        if($fullMode) {
          ...
          <br/><p class=\"ShoprocketCustomFieldDesc\">$desc:<br/><strong>$brValue</strong><br/>$change</p>
          <div id='customForm_$itemIndex' style='display: $showCustomForm;'>
          <textarea name=\"customFieldInfo[$itemIndex]\" class=\"ShoprocketCustomTextarea\" id=\"custom_field_info_$itemIndex\" />$value</textarea>
  ...
  public function getProductPrice() {
    ...
    elseif($product->is_user_price == 1 || $product->gravity_form_pricing) {
      ...
      //ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Looking for product price in session variable: " . $session_var_name . "\n" . 
      //  print_r(ShoprocketSession::dump(), true));
        
      if(ShoprocketSession::get($session_var_name)){
        // using a user-defined price
        $userPrice = ShoprocketSession::get($session_var_name);
  ...
  public function getBaseProductPrice(){
    ...
    if(false && !empty($product->gravity_form_id) && $product->gravity_form_pricing == 1){
      // gravity form price
      $price = ShoprocketGravityReader::getPrice($this->getFirstFormEntryId()) / $this->getQuantity();
      ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Gravity Form product price: $price");
    }
    ...
    if($this->_productId > 0) {
      ...
      elseif($product->is_user_price == 1 || $product->gravity_form_pricing) {
        ...
        if(ShoprocketSession::get($session_var_name)) {
          ...
          $priceDescription = ShoprocketCommon::currency($userPrice);
        }
        else{
          $priceDescription = ShoprocketCommon::currency($product->price);
        }
      }
      else {
  ...
  public function isEqual(ShoprocketCartItem $item) {
    $isEqual = true;
    ...
    if(is_array($this->_formEntryIds)) {
      foreach($this->_formEntryIds as $entryId) {
        ...
        if($fullMode) {
          ...
          $removeLink .= 'shoprocket-task=remove-attached-form&entry=' . $entryId;
          $removeLink = '<a class="ShoprocketRemoveFormLink" href="' . $removeLink . '">remove</a>';
        }
        */
        $out .= "<div class='ShoprocketGravityFormDisplay'>" . ShoprocketGravityReader::displayGravityForm($entryId) . "</div>";
      }
    }
    return $out;
  }
  ...
  public function detachFormEntry($lead_id) {
    ...
    ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Start to detach gravity forms: " . print_r($entries, true));
    
    if(in_array($lead_id, $entries)) {
      ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Lead id is in the list of entries: " . $lead_id);
