<?php
abstract class ShoprocketModelAbstract extends ShoprocketBaseModelAbstract {
  ...
  public function getModels($where=null, $orderBy=null, $limit=null, $tableName=null, $id=null) {
    ...
    // ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] " . get_class($this) . " getModels: $sql");
  ...
  public function getOne($where='where id>0', $orderBy=null) {
    ...
    if(count($models)) {
      $model = $models[0];
      ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Get one is returning: " . count($models));
    }
    return $model;
  }
  ...
  public function save() {
    ...
    ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] " . get_class($this) . " Saving Data: " . print_r($this->_data, true));
  ...
  protected function _insert() {
    if(isset($this->_data['created_at'])) {
      $this->_data['created_at'] = date('Y-m-d H:i:s', ShoprocketCommon::localTs());
    }
    if(isset($this->_data['updated_at'])) {
      $this->_data['updated_at'] = date('Y-m-d H:i:s', ShoprocketCommon::localTs());
    }
