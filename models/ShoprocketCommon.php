...
class ShoprocketCommon {
  ...
  public static function getButtonId($id) {
    global $shoprocketCartButtons;

    $idSuffix = '';

    if(!is_array($shoprocketCartButtons)) {
      $shoprocketCartButtons = array();
    }

    if(in_array($id, array_keys($shoprocketCartButtons))) {
      $shoprocketCartButtons[$id] += 1;
    }
    else {
      $shoprocketCartButtons[$id] = 1;
    }

    if($shoprocketCartButtons[$id] > 1) {
      $idSuffix = '_' . $shoprocketCartButtons[$id];
    }
  ...
  public static function getView($filename, $data=null, $notices=true, $minify=false) {
    ...
    if(strpos($filename, 'admin') !== false) {
      $mijireh_notice = ShoprocketSetting::getValue('mijireh_notice');
      if($mijireh_notice != 1) {
        if($notices) {
          ...
          <a href="#" id="mijireh_dismiss" class="close">&times;</a>
            <div class="mijireh-logo"><img src="' . SHOPROCKET_URL . '/images/shoprocket-logo.jpg" alt="Mijireh Checkout Logo"></div>
            <div class="mijireh-blurb">
            <h2>shoprocket A cloud based ecommerce solution</h2>
            ...
            <h1><a href="http://shoprocket.co" target="_new">Start now for FREE!</a></h1>
            </div>
          </div>';
        }
      }
    ...
    $overrideDirectory = $themeDirectory."/shoprocket-templates";
    $userViewFile = $overrideDirectory."/$filename";
    
    //ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Override: $overrideDirectory\nUser view file: $userViewFile");
    ...
    if(file_exists($userViewFile) && in_array($filename,$approvedOverrideFiles)) {
      ...
      if(filesize($userViewFile)>10) {
        ...
        ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] User file was empty: $userViewFile");
      }
    }
    else{
      // ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] File exists: ".var_export(file_exists($userViewFile),true)."\n");
      // ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Approved Override: ".var_export(in_array($filename,$approvedOverrideFiles),true));
    }
  
    // Check for override and confirm we have a registered plugin
    if($customView && false && self::isRegistered()) {
      // override is present
      $filename = $customViewPath;
    }
    else {
      // no override, render standard view
      $filename = SHOPROCKET_PATH . "/$filename";
    }
    ...
    return ($minify) ? ShoprocketCommon::minifyMarkup($notice . $contents) : $notice . $contents;
  }
  ...
  public static function getTableName($name, $prefix='shoprocket_'){
      global $wpdb;
  ...
  public static function getTablePrefix(){
      global $wpdb;
      return $wpdb->prefix . "shoprocket_";
  }
  
  /**
   * If Shoprocket_DEBUG is defined as true and a log file exists in the root of the Shoprocket plugin directory, log the $data
   */
  public static function log($data) {
    
    if(defined('Shoprocket_DEBUG') && Shoprocket_DEBUG) {
  ...
  public static function isLoggedIn() {
    $isLoggedIn = false;
    if(ShoprocketSession::get('ShoprocketAccountId') && is_numeric(ShoprocketSession::get('ShoprocketAccountId')) && ShoprocketSession::get('ShoprocketAccountId') > 0) {
      $isLoggedIn = ShoprocketSession::get('ShoprocketAccountId');
    }
    return $isLoggedIn;
  }
  ...
  public static function awardCommission($orderId, $referrer) {
    global $wpdb;
    if (!empty($referrer)) {
      $order = new ShoprocketOrder($orderId);
      if($order->id > 0) {
        $subtractAmount = 0;
  ...
  public static function isValidEmail($email) {
    ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Checking to see if email address is valid: $email");
    ...
    if(!empty($email) && strlen($email) > 3) {
      if(preg_match($pattern, $email)) {
        ...
        ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Email validation failed because address is invalid: $email");
      }
    }
    else {
      ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Email validation failed because address is empty: $email");
    }
    return $isValid;
  }
  ...
  public static function getHomeCountryCode() {
    if($homeCountry = ShoprocketSetting::getValue('home_country')) {
  ...
  public static function getShippingCountries() {
    ...
    $use_array = false;
    if($selected_country = ShoprocketSession::get('ShoprocketShippingCountryCode')) {
  ...
  public static function getCountries($all=false) {
    ...
    $setting = new ShoprocketSetting();
    $home_country = ShoprocketSetting::getValue('home_country');
    if($home_country) {
    ...
    if($all) {
      ...
      $international = ShoprocketSetting::getValue('international_sales');
      if($international) {
        if($customCountries) {
          //ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Got some custom countries: " . print_r($customCountries, true));
          $countries = $customCountries;
        }
      }
      else {
  ...
  public static function getCustomCountries() {
    $list = false;
    $setting = new ShoprocketSetting();
    $countries = ShoprocketSetting::getValue('countries');
    if($countries) {
  ...
  public static function getOrderStatusOptions() {
    $statuses = array();
    $setting = new ShoprocketSetting();
    $opts = ShoprocketSetting::getValue('status_options');
    if(!empty($opts)) {
      $opts = explode(',', $opts);
      foreach($opts as $o) {
        $statuses[] = trim($o);
  ...
  public static function getPromoMessage() {
    $promo = ShoprocketSession::get('ShoprocketPromotion');
    $promoMsg = "none";
    if($promo) {
      $promoMsg = $promo->getCode() . ' (-' . ShoprocketCommon::currency(ShoprocketSession::get('ShoprocketPromotion')->getDiscountAmount(ShoprocketSession::get('ShoprocketCart')), false) . ')';
    }
    return $promoMsg;
  }
  ...
  public function showErrors($errors, $message=null) {
    $out = "<div id='shoprocketErrors' class='ShoprocketError'>";
    if(empty($message)) {
      $out .= "<p><b>" . __("We're sorry.<br/>Your order could not be completed for the following reasons","shoprocket") . ":</b></p>";
    }
    else {
      $out .= $message;
    }
...
<script type="text/javascript">
  (function($){
    ...
    $lines  = rtrim($lines, "\n");
    ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] jq error script lines: $lines");
  ...
  public static function getPayPalUrl() {
    ...
    if(ShoprocketSetting::getValue('paypal_sandbox')) {
  ...
  public static function curl($url, $method='GET') {
    ...
    if (!function_exists('curl_init')){ 
      throw new ShoprocketException('cURL is not installed!');
    }
  ...
  public static function downloadFile($path) {
    ...
    // Validate the $path
    if(!strpos($path, '://')) {
      if($productFolder = ShoprocketSetting::getValue('product_folder')) {
        if(strpos($path, $productFolder) === 0) {
          ...
          ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Download file name: $fileName");
          ...
          if(substr($path, 0, 4) == 'http') {
            $bytes = ShoprocketCommon::remoteFileSize($path);
          }
          else {
            $bytes = filesize($path);
          }
          ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Download file size: $bytes");
          ...
          if($fp = fopen($path, 'rb')) {
            ...
            ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] fopen failed to open path: $path");
          }
        }
        else {
          ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Unable to download file because the requested file is not in the path defined by the product folder settings: $path");
        }
      }
      else {
        ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Unable to download file because the product folder is not set.");
      }
    }
  ...
  public static function onlyUsingPayPalStandard() {
    ...
    if(ShoprocketSetting::getValue('paypal_email')) {
      $onlyPayPalStandard = true;
    }
    
    if(ShoprocketSetting::getValue('auth_username') || ShoprocketSetting::getValue('paypalpro_api_username')) {
      $onlyPayPalStandard = false;
    }
  ...
  */
  ...
  public static function showReportData(){
    ...
    $output .= (false) ? "Shoprocket Version: Pro " . ShoprocketSetting::getValue('version') . "<br>" : "Shoprocket Version: " .ShoprocketSetting::getValue('version') . "<br>";
  ...
  public static function getElapsedTime($datestamp) {
    ...
    if(!empty($datestamp) && $datestamp != '0000-00-00 00:00:00') {
      $totaldelay = ShoprocketCommon::localTs() - strtotime($datestamp);
      if(ShoprocketCommon::localTs() == strtotime($datestamp)) {
        $output = __('Now', 'shoprocket');
      }
      ...
      elseif($hours=floor($totaldelay/3600)) {
        ...
        $output = $hours . ' ' . _n('hour', 'hours', $hours, 'shoprocket') . __(" ago","shoprocket");
      }
      elseif($minutes=floor($totaldelay/60)) {
        $totaldelay = $totaldelay % 60;
        $output = $minutes . ' ' . _n('minute', 'minutes', $minutes, 'shoprocket') . __(" ago","shoprocket");
      }
      elseif($seconds=floor($totaldelay/1)) {
        $totaldelay = $totaldelay % 1;
        $output = $seconds . ' ' . _n('second', 'seconds', $seconds, 'shoprocket') . __(" ago","shoprocket");
      }
    }
    return $output;
  }
  ...
  public static function getTimeLeft($datestamp) {
    $output = false;
    if(!empty($datestamp) && $datestamp != '0000-00-00 00:00:00') {
      $timeleft = strtotime($datestamp) - ShoprocketCommon::localTs();
      if(ShoprocketCommon::localTs() == strtotime($datestamp)) {
        $output = __('Now', 'shoprocket');
      }
      ...
      elseif($hours=floor($timeleft/3600)) {
        ...
        $output = $hours . ' ' . _n('hour', 'hours', $hours, 'shoprocket') . __(" to go","shoprocket");
      }
      elseif($minutes=floor($timeleft/60)) {
        $timeleft = $timeleft % 60;
        $output = $minutes . ' ' . _n('minute', 'minutes', $minutes, 'shoprocket') . __(" to go","shoprocket");
      }
      elseif($seconds=floor($timeleft/1)) {
        $timeleft = $timeleft % 1;
        $output = $seconds . ' ' . _n('second', 'seconds', $seconds, 'shoprocket') . __(" to go","shoprocket");
      }
    }
    return $output;
  }
  
  public static function shoprocketUserCan($role) {
    $access = false;
  ...
  public static function displayVersionInfo() {
    if(false) {
      echo '<meta name="generator" content="Shoprocket Professional ' . ShoprocketSetting::getValue('version') . '" />' . "\n";
    }
    else {
      echo '<meta name="generator" content="Shoprocket Lite ' . ShoprocketSetting::getValue('version') . '" />' . "\n";
    }
  }
  
  public static function removeShoprocketMeta() {
    remove_action('wp_head', array('ShoprocketCommon','displayVersionInfo'));
  }
  ...
  public static function isSlurpPage() {
    ...
    if(isset($post) && is_object($post)) {
      ...
      // ShoprocketCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Check Slurp Page Failed: " . print_r($post, 1));
    }
    return $isSlurp;
  }
  ...
  public static function sessionType() {
    $type = ShoprocketSetting::getValue('session_type');
    if(!$type) {
      $type = 'database';
    }
    return $type;
  ...
  public static function cleanNumber($string) {
    $dec_point = ShoprocketSetting::getValue('currency_dec_point') ? ShoprocketSetting::getValue('currency_dec_point') : '.';
  ...
  public static function verifyCartPages($outputType = 'full'){
    ...
    foreach($requiredPages as $page => $slug){
       ...
       if($wordpress_page && !in_array($wordpress_page->post_status, array('trash','draft','private'))){
         $success[] = "<li class='ShoprocketPageSuccess'><strong>$page page found</strong></li>";
       }
       else{
         $error[] = "<li class='ShoprocketPageError'><strong>$page page not found!</strong> The page should be located at " . get_bloginfo('url') . "/$slug</li>";
       }
    }
  ...
  public static function convert_currency_to_number($amount) {
    ...
    $decimal = ShoprocketSetting::getValue('currency_decimals') == 'no_decimal' ? 0 : (ShoprocketSetting::getValue('currency_decimals') ? ShoprocketSetting::getValue('currency_decimals') : 2);
    $dec_point = ShoprocketSetting::getValue('currency_dec_point') ? ShoprocketSetting::getValue('currency_dec_point') : '.';
    $thousands_sep = ShoprocketSetting::getValue('currency_thousands_sep') ? ShoprocketSetting::getValue('currency_thousands_sep') : ',';
  ...
  public static function currency($amount, $html=true, $markup=false, $symbol=true) {
    ...
    if($markup) {
      ...
      if($symbol) {
        if($html) {
          if(ShoprocketSetting::getValue('currency_position') == 'after') {
            $amount = self::currencyFormat($amount, $decimal, $dec_point, $thousands_sep) . Shoprocket_CURRENCY_SYMBOL;
          }
          else {
            $amount = Shoprocket_CURRENCY_SYMBOL . self::currencyFormat($amount, $decimal, $dec_point, $thousands_sep);
          }
        }
        else {
          if(ShoprocketSetting::getValue('currency_position') == 'after') {
            $amount = self::currencyFormat($amount, $decimal, $dec_point, $thousands_sep) . Shoprocket_CURRENCY_SYMBOL_TEXT;
          }
          else {
            $amount = Shoprocket_CURRENCY_SYMBOL_TEXT . self::currencyFormat($amount, $decimal, $dec_point, $thousands_sep);
          }
        }
      }
      else {
 ...
 /* public static function tax($rate) {
  ...
  public static function currencyMarkup($amount) {
    $amount = str_replace(Shoprocket_CURRENCY_SYMBOL, '', $amount);
    $decimal = ShoprocketSetting::getValue('currency_decimals') == 'no_decimal' ? 0 : (ShoprocketSetting::getValue('currency_decimals') ? ShoprocketSetting::getValue('currency_decimals') : 2);
    $dec_point = ShoprocketSetting::getValue('currency_dec_point') ? ShoprocketSetting::getValue('currency_dec_point') : '.';
    $thousands_sep = ShoprocketSetting::getValue('currency_thousands_sep') ? ShoprocketSetting::getValue('currency_thousands_sep') : ',';
    ...
    if(is_numeric($amount)) {
      ...
      $html = '<span class="ShoprocketCurrencySymbol ShoprocketCurrencySymbolbefore">' . self::currencySymbol('before') . '</span>';
      $html .= '<span class="ShoprocketPreDecimal">' . number_format($preDecimal, 0, '', $thousands_sep) . '</span><span class="ShoprocketDecimalSep">' . $dec_point . '</span><span class="ShoprocketPostDecimal">' . $postDecimal . '</span>';
      $html .= '<span class="ShoprocketCurrencySymbol ShoprocketCurrencySymbolAfter">' . self::currencySymbol('after') . '</span>';
    }
    else {
      $html = $amount;
    }
  ...
  public static function currencySymbol($position, $html=true) {
    $symbol = '';
    if(ShoprocketSetting::getValue('currency_position')) {
      if($position == ShoprocketSetting::getValue('currency_position') && $html) {
        $symbol = Shoprocket_CURRENCY_SYMBOL;
      }
      elseif($position == ShoprocketSetting::getValue('currency_position') && !$html) {
        $symbol = Shoprocket_CURRENCY_SYMBOL_TEXT;
      }
    }
    else {
